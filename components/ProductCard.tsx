...
import { Calendar, BookOpen, FileText, Edit3, Trash2 } from 'lucide-react';
...
const ProductCard: React.FC<ProductCardProps> = ({ product: album, onClick, isManageMode, onEdit, onDelete }) => {
  ...
  return (
    <div 
      className={`group relative bg-white rounded-xl shadow-sm border hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden flex flex-col h-full ${isManageMode ? 'border-indigo-500 ring-2 ring-indigo-500/20' : 'border-slate-100'}`}
        ...
        {isManageMode ? (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center gap-3 animate-in fade-in duration-200">
                <button
                    ...
                    className="p-3 bg-white text-slate-900 rounded-full hover:bg-indigo-600 hover:text-white transition-colors shadow-lg transform hover:scale-110"
                    title="编辑"
                >
                    <Edit3 size={20} />
                </button>
